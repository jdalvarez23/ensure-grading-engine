 ...
 * @type {Element}
 */
var metaTag = document.querySelector('meta[name="ensure-grader"]');
...
function importFeedbackWidget() {
  var twScript = document.querySelector('script#ensure-test-widget');

  if (!twScript) {
    return injectIntoDocument('script', {
      src: chrome.extension.getURL('app/templates/templates.js'),
      id: 'ensure-test-widget'
    }, 'head');
  } else {
    return Promise.resolve();
  }
...
function registerTestSuites(json) {
  ...
  if (errorMsg) {
    ...
    return injectIntoDocument('script', {
      text: 'ensureFEGradingEngine.registerSuites(' + json + ');'
    }, 'head');
  }
}
...
function turnOn() {
  ...
  return injectIntoDocument('script', {
    ...
    innerHTML: 'ensureFEGradingEngine.turnOn();'
  }, 'head');
}
